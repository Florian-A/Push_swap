...
static int	test_pa1(void)
{
	...
	if (!(((t_block *)push_swap.stack[0]->content)->nb == 0
			&& ((t_block *)push_swap.stack[1]->content)->nb == 1
...
static int	test_pa2(void)
{
	t_push_swap	push_swap;

	init_s_stack(&push_swap);
	auto_fill_stack(&push_swap, 0, 4);
...
static int	test_pa4(void)
{
	...
	pa(&push_swap);
	if (!(((t_block *)push_swap.stack[0]->content)->nb == 0
			&& ((t_block *)push_swap.stack[0]->next->content)->nb == 1
			&& ((t_block *)push_swap.stack[0]->next->next->content)->nb == 2))
	{
		cleaning(&push_swap);
		return (-1);
	}
